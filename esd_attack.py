...
def esd_attack (target, messages, batch_size, csv):
	...
	for i in range(0,26):
		for j in range(0,10):
			...
			obs_dist[chr(ord('a') + i) + str(j)] = float(0)
			back_dist[chr(ord('a') + i) + str(j)] = float(0)
...
messages = read_file_part_1.GetFileInfo("rounds.csv", "'", (", ", " ", "\n", "]"))

csv = open("p1-friends.csv", "w")

#For each target (a0, b0, ..., z0) run the extended statistical disclosure attack
for i in range(0,26):
	target = chr(ord('a') + i) + '0'
	esd_attack(target, messages, 32, csv)
